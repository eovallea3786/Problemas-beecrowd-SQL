...
SELECT lif.name, ROUND(lif.omega * 1.618, 3) "The Factor N"
FROM life_registry lif
INNER JOIN dimensions dim
ON lif.dimensions_id = dim.id
WHERE lif.name LIKE 'Richard%' AND (dim.name = 'C875' OR dim.name = 'C774')
ORDER BY lif.omega